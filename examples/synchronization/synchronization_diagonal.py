...
class NeuromodulatorModel(NetpyneModel, ProducesSyncMeasure):
    def __init__(self, name, nmda_weight=0.7, gaba_weight=0.005):
        super().__init__(
            name=name,
            backend=("Netpyne", {"use_memory_cache": False, "use_disk_cache": False}),
        )
...
neuromodulators_suite = sciunit.TestSuite(
    [default_test, nmda_test, gaba_test], name="Neuromodulators test suite"
)
score_matrix = neuromodulators_suite.judge(
    [model_by_default, model_with_nmda_antagonist, model_with_gaba_agonist]
)
print(score_matrix)
