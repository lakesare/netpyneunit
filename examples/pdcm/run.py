...
def fast_example_please():
    model_2 = PdcmModel(
        name="2%", scale=0.02, ext_input="balanced_poisson", duration=1 * 1e3
    )
    model_1 = PdcmModel(
        name="1%", scale=0.01, ext_input="balanced_poisson", duration=1 * 1e3
    )
    ...
    table = suite.judge([model_2, model_1])
    print(table)
