...
class PdcmModel(NetpyneModel, ProducesMeanFiringRate):
    def __init__(self, name, scale, ext_input, duration=60 * 1e3):
        super().__init__(
            name=name,
            backend=("Netpyne", {"use_memory_cache": True, "use_disk_cache": False}),
        )
        self.scale = scale
    ...
    def getSimConfig(self):
        ...
        cfg.simLabel = "pd_scale-%s_DC-%d_TH-%d_Balanced-%d_dur-%d_Seed-%d" % (
            str(cfg.ScaleFactor),
            int(cfg.DC),
            int(cfg.TH),
            int(cfg.Balanced),
        ...
        cfg.analysis["plotSpikeStats"] = {
            ...
            "stats": ["rate", "isicv"],
            "legendLabels": cfg.recordCellsSpikes,
            "timeRange": [100, cfg.duration],
            "fontSize": 16,
            "figSize": (6, 9),
            "showFig": False,
            "saveFig": True,
    ...
    def getNetParams(self, cfg):
        ...
        def Reescale(ScaleFactor, C, N_Full, w_p, f_ext, tau_syn, Inp, InpDC):
            if ScaleFactor < 1.0:
                # 1
                F_out = np.array(
                    [0.860, 2.600, 4.306, 5.396, 8.142, 8.188, 0.941, 7.3]
                )  # Good approximation
                ...
                InpDC = (
                    np.sqrt(ScaleFactor) * InpDC * w_p * f_ext * tau_syn * 0.001
                )  # pA
    ...
    Vteta  = -50 (mV)"""
        ...
        InpDC, N_, w_p = Reescale(
            cfg.ScaleFactor, C, N_Full, w_p, f_ext, tau_syn, Inp, InpDC
        )
        ...
        for i in range(0, 8):
            netParams.popParams[L[i]] = {
                "cellType": str(L[i]),
                "numCells": int(N_[i]),
                "cellModel": "IntFire_PD",
                "m": 0,
                "Iext": float(InpDC[i]),
            }

        # To atualization of Point Neurons
        netParams.popParams["bkg_IF"] = {
            "numCells": 1,
            "cellModel": "NetStim",
            "rate": 40000,
            "start": 0.0,
